...
produced with `asr-diar-bench`. Use it to pick a backend; reproduce it on your
...
python -m asr_diar_server.bench.utils.make_ami_clip IB4001 \
...
ASR_DIAR_BACKEND_ASR=faster-whisper ASR_DIAR_MODEL_ASR=openai/whisper-large-v3-turbo \
ASR_DIAR_ASR_DEVICE=cuda ASR_DIAR_ASR_COMPUTE_TYPE=float16 \
ASR_DIAR_BACKEND_DIARIZATION=nemo ASR_DIAR_PIPELINE=full-memory \
  asr-diar-server --port 8123

# 3) Run the full benchmark (quality + performance):
asr-diar-bench all --clips-dir clips --server-url http://127.0.0.1:8123 \
