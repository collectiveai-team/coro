...
The packaged server uses a root-level `asr_diar_server` package, exposes a lightweight module-level `asr_diar_server.app:app`, and exposes one public transcription route: `/v1/audio/transcriptions`. Full-memory and chunked-file behavior are not public API versions; they are startup-selected transcription pipeline implementations chosen by `ASR_DIAR_PIPELINE`.
...
Core interfaces live in the core boundary. `ASRAdapter`, `DiarizationAdapter`, and `TranscriptionPipeline` protocols are defined under `asr_diar_server.core`, while provider implementations live under `asr_diar_server.backends`. Backend integrations are provider-first rather than capability-first so one provider module can own ASR and diarization integration without leaking backend-native objects into pipelines or response schemas.
