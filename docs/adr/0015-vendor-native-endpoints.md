...
(ADR 0014), and discarded both at the API boundary. Per-word speakers are now
...
flag from ADR 0014 already covers the narrower case of concurrent speakers.

## Speaker labels: absent, not invented; numbered, not renumbered

**The `-1` sentinel becomes an absent key.** ADR 0014 uses `-1` for a word the
...
STM used for cpWER, DI-cpWER and DER scoring (ADR 0014), so translating here
...
The withdrawn draft that ADR 0014 supersedes made the two coincide, because it
split segments at every word-level speaker change. ADR 0014 removed that split:
segments are sentence-shaped and carry the duration-weighted majority of their
own words, so a segment now *is* a summary over several speakers and therefore
an invalid utterance. Deriving turns from the per-word truth is what keeps this
correct, and means the per-word view and the turn view cannot disagree, because
...
and is ~0.2% of pipeline time (ADR 0014).
