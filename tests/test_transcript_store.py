...
def test_store_round_trips_segments_in_order(tmp_path):
    with TranscriptSpillStore(directory=str(tmp_path)) as store:
        store.append_segment(_segment(0.0, 1.0, "hola.", "1", [{"word": "hola."}]))
    ...
    assert segments[0]["words"] == [{"word": "hola."}]
...
def test_store_round_trips_raw_words_in_order(tmp_path):
    with TranscriptSpillStore(directory=str(tmp_path)) as store:
        store.append_raw_words(
            [
                {"word": " hola", "start": 0.0, "end": 0.5, "score": 0.9},
                {"word": " mundo", "start": 0.5, "end": 1.0, "score": 0.8},
            ]
        )
        store.append_raw_words([{"word": " !", "start": 1.0, "end": 1.1, "score": 1.0}])
